...
print(f"\nTime\t\tFlag0\t\tFlag4\t\tPossible solutions")
...
for a in block1:
    ...
    for b in block2:
        ...
        if block2.index(b)<=block1.index(a):
            #print(f"{sf.time_diff(start_time):06.2f}",end="\r")
            continue
                ...
                if block3.index(c)<=block2.index(b):
                    ...
                    for d in block4:
                        ...
                        if block4.index(d)<=block3.index(c):
                            #print(f"{sf.time_diff(start_time):06.2f}",end="\r")
                            continue
                        elif sf.index_combined(w,z) in compatible_blocks and sf.index_combined(x,z) in compatible_blocks and sf.index_combined(y,z) in compatible_blocks:
                            flag4+=1          
                            ...
                            if sos<=sos1:
                                sos1=sos
                                dict2={"Soln_rank": flag4,"List0":a, "List1":b,"List2":c, "List3":d,"Flags":sos,"Position":[w,x,y,z]}
                            ...
                            print(f"{time_di:06.2f}\t\t{flag0:0>3,d}\t\t{flag4:0>6,d}\t\t{w :03} {x :03} {y :03} {z :03} {sos :02} {flag0/len(master_list)*100:04.1f}%", end="\r")
